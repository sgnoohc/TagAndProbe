...
static int load_runs (const char *fname, enum file_type type)
{
     ...
     do {
          ...
          // there is a newline)
          s = fscanf(file, "%1024[^\n]%n", buf, &n);
          if (n >= 1023) {throw std::runtime_error("[goodrun] Error: n exceeds buffer size");}
          assert(n < 1023);
          if (s != 1) {
               if (s != EOF) {
               ...
               char *pbuf = buf;
                   ...
                   if (s != 1) {throw std::runtime_error(Form("Expected a run number (unsigned int) in the first position of line %d: %s\n", line, buf));}
               if (s != 1) {
               ...
               if (strlen(trail) != 0) {throw std::runtime_error(Form("Unexpected trailing junk (%s) on line %d: %s\n", trail, line, buf));}
               if (strlen(trail) != 0) {
...
bool goodrun_json(const unsigned int run, const unsigned int lumi_block)
{
    if (not good_runs_loaded_) {
        ...
        if (loaded) {throw std::runtime_error("[goodrun] Error: %s not loaded!");}
        assert(!loaded);
        good_runs_loaded_ = loaded;
    }
...
void set_goodrun_file (const char* filename)
{
    ...
    if (loaded) {throw std::runtime_error(Form("[goodrun] Error: %s not loaded!", filename));}
    assert(!loaded);
    good_runs_loaded_ = !loaded;
}
